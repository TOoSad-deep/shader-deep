...
def save_run(directory: Path, state: BlackboardState, details: Mapping[str, object]) -> None:
    ...
    temporary.write_text(json.dumps({**details, "blackboard": board}, ensure_ascii=False, indent=2), encoding="utf-8")
    # 先写完同目录临时文件再替换, 避免读取者看到只写了一半的 run.json.
    # 该操作保护单个快照文件, 不把 GLSL、PNG 和 JSON 的写入变成一个事务.
