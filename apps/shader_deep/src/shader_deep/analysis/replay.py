...
from shader_deep.analysis.exploration import ExplorationReport, PossibilityLibrary, VisualOutline, validate_outline
from shader_deep.analysis.exploration_session import PROTOCOL, ExplorationSession
...
from shader_deep.analysis.types import MIN_ANALYSIS_TASKS, AnalysisExecution, AnalysisOptions
...
if TYPE_CHECKING:
    from shader_deep.analysis.types import AnalysisOutcome
    from shader_deep.schemas import BlackboardState

_Session = TypeVar("_Session", bound=ExplorationSession)
...
def _write_manifest(session: ExplorationSession, source: Path, frozen: ReplayInput) -> None:
...
def prepare_replay(source: Path, *, options: AnalysisOptions) -> ExplorationSession:
