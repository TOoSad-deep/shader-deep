...
from shader_deep.analysis.exploration import ExplorationReport, OutlineElement, VisualOutline
from shader_deep.analysis.exploration_worker import run_exploration
from shader_deep.analysis.types import AnalysisOptions
from shader_deep.context.exploration import build_exploration_context, build_integration_context, build_outline_context
...
class ExplorationWorkerTests(GenerationFixture):
    ...
    def test_commit_replay_is_idempotent_and_changed_payload_is_rejected(self) -> None:
        ...
        with patch("shader_deep.analysis.exploration_worker.AnalysisLoop.run", autospec=True, side_effect=run):
            outcome = self.run_worker(commit_report=commit)
