...
from shader_deep.workflows.replay import load_replay_input, prepare_replay, run_replay
...
class AnalysisReplayTests(GenerationFixture):
    ...
    def test_service_failure_preserves_error_and_original_inputs(self) -> None:
        with patch("shader_deep.workflows.replay.build_model", side_effect=RuntimeError("fixture unavailable")):
            outcome = run_replay(self.source, options=self.options)
