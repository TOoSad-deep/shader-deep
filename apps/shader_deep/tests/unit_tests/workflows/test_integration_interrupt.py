...
class IntegrationInterruptTests(GenerationFixture):
    ...
    def test_keyboard_interrupt_reraises_after_saving_partial_library_and_draft(self) -> None:
        ...
        with (
            patch("shader_deep.compatibility.ondemand.session.AnalysisLoop.run", autospec=True, side_effect=self._run_loop),
            patch("shader_deep.workflows.dispatch.run_exploration", side_effect=self._worker),
